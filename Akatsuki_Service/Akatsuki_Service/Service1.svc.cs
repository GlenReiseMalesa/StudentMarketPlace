...
namespace Akatsuki_Service
{
    ...
    public class Service1 : IService1
    {
        ...
        {
            dynamic SS = (from m in db.Students where (ID.Equals(m.Student_Number)) select m).FirstOrDefault();
            var student = new Student
            {


                Student_Number = SS.Student_Number,
                Student_Address = SS.Student_Address,
                Student_Cellphone_Number = SS.Student_Cellphone_Number,
                Student_Email = SS.Student_Email,
                Student_Surname = SS.Student_Surname,
                Student_Name = SS.Student_Name,
                Student_Personal_Email= SS.Student_Personal_Email,
                Student_Username= SS.Student_Username,
        ...
        public bool UpdateUserDetails(int sNum, string sAddress, string sEmail, string personalEmail, int cellphoneNum, string fName, string lName, string password)
        {
            var userDetails = (from u in db.Students
                               where sNum.Equals(u.Student_Number)
                               select u).FirstOrDefault();

           


            userDetails.Student_Number = sNum;
            userDetails.Student_Username = "";
            userDetails.Student_Address = sAddress;
            userDetails.Student_Email = sEmail;
            userDetails.Student_Name = fName;
            userDetails.Student_Personal_Email = personalEmail;
            userDetails.Student_Surname = lName;
            userDetails.Student_Cellphone_Number = cellphoneNum;
        ...
        public bool editGoodItem(int goodID, string gName, decimal gPrice, string gdescription)
        {
            var goodDetails = (from u in db.Goods
                               where goodID.Equals(u.Goods_ID)
                               select u).FirstOrDefault();

            goodDetails.Goods_ID = goodID;
            goodDetails.Student_Number = goodDetails.Student_Number;
            goodDetails.Goods_Name = gName;
            goodDetails.Goods_Price = gPrice;
            goodDetails.Goods_Description = gdescription;
            goodDetails.Goods_Image = goodDetails.Goods_Image;
            goodDetails.Goods_Catergory = goodDetails.Goods_Catergory;
        ...
        public bool editServiceItem(int serviceID, string sName, decimal sPrice, string sdescription)
        {
            var serviceDetails = (from u in db.Services
                               where serviceID.Equals(u.Services_ID)
                               select u).FirstOrDefault();

            serviceDetails.Services_ID = serviceID;
            serviceDetails.Student_Number = serviceDetails.Student_Number;
            serviceDetails.Services_Name = sName;
            serviceDetails.Services_Price = sPrice;
            serviceDetails.Services_Description = sdescription;
            serviceDetails.Services_Image_ = serviceDetails.Services_Image_;
            serviceDetails.Services_Catergory = serviceDetails.Services_Catergory;
        ...
        public bool Update_Cart_Product(int CartId, int ItemQuantity)
        {
            var Cart_Product = (from u in db.ProductCarts
                                where CartId.Equals(u.CartID)
                                select u).FirstOrDefault();

            Cart_Product.CartID = Cart_Product.CartID;
            Cart_Product.ProductID = Cart_Product.ProductID;
            Cart_Product.ItemQuantity = ItemQuantity;
        ...
        public bool Update_Cart_Service(int CartId,int ItemQuantity)
        {
            var Cart_Service = (from u in db.ServiceCarts
                                  where CartId.Equals(u.CartID)
                                  select u).FirstOrDefault();

            Cart_Service.CartID= Cart_Service.CartID;
            Cart_Service.ServiceID= Cart_Service.ServiceID;
            Cart_Service.ItemQuantity = ItemQuantity;
